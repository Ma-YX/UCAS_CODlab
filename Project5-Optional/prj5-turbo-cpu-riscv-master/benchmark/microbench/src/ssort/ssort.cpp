...
void suffixArray(int* s, int* SA, int n, int K) {
  int n0=mdiv((n+2),3).q, n1=mdiv((n+1),3).q, n2=mdiv(n,3).q, n02=n0+n2; 
  ...
  for (int i=0, j=0;  i < n+(n0-n1);  i++) if (mdiv(i,3).r != 0) s12[j++] = i;
  ...
  for (int i = 0;  i < n02;  i++) {
    ...
    if (mdiv(SA12[i] , 3).r == 1) { s12[mdiv(SA12[i],3).q]      = name; } // left half
    else                  { s12[mdiv(SA12[i],3).q + n0] = name; } // right half
  }
  ...
  for (int i=0, j=0;  i < n02;  i++) if (SA12[i] < n0) s0[j++] = mmul(3,SA12[i]);
  radixPass(s0, SA0, s, n0, K);

  // merge sorted SA0 suffixes and sorted SA12 suffixes
  for (int p=0,  t=n0-n1,  k=0;  k < n;  k++) {
#define GetI() (SA12[t] < n0 ? mmul(SA12[t] , 3) + 1 : mmul((SA12[t] - n0) , 3) + 2)
    int i = GetI(); // pos of current offset 12 suffix
    int j = SA0[p]; // pos of current offset 0  suffix
    if (SA12[t] < n0 ? 
        leq(s[i],       s12[SA12[t] + n0], s[j],       s12[mdiv(j,3).q]) :
        leq(s[i],s[i+1],s12[SA12[t]-n0+1], s[j],s[j+1],s12[mdiv(j,3).q+n0]))
