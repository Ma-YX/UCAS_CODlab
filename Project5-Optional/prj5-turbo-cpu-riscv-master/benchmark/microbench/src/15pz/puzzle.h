  ...
  for ( int i = 0; i < N; ++i ) {
    for ( int j = 0; j < N; ++j ) {
      ...
      if ( array[k] == 0 ) {
        ...
        mdiv_t t = mdiv(array[k] - 1, N);
        manhattan_distance += abs(t.q - i );
        manhattan_distance += abs(t.r - j );
      }
...
void N_puzzle<N>::determine_hash() {
  ...
  for ( int i = 0; i < N; ++i ) {
    for ( int j = 0; j < N; ++j ) {
      hash_value = mmul(hash_value,1973) + puzzle[i][j];
    }
  }
}
...
N_puzzle<N> N_puzzle<N>::tile_up() const {
  ...
  result.manhattan_distance +=
    abs( mdiv((puzzle[zero_i + 1][zero_j] - 1), N).q - zero_i ) -
    abs( mdiv((puzzle[zero_i + 1][zero_j] - 1), N).q - (zero_i + 1) );
...
N_puzzle<N> N_puzzle<N>::tile_left() const {
  ...
  result.manhattan_distance +=
    abs( mdiv((puzzle[zero_i][zero_j + 1] - 1) , N).r - zero_j ) -
    abs( mdiv((puzzle[zero_i][zero_j + 1] - 1) , N).r - (zero_j + 1) );
...
bool N_puzzle<N>::solvable() const {
  ...
  for ( int i = 0; i < N; ++i ) {
    for ( int j = 0; j < N; ++j ) {
      if ( puzzle[i][j] == 0 ) {
        entries[mmul(N,i) + j] = N*N; 
      } else {
        entries[mmul(N,i) + j] = puzzle[i][j];
      }
    }
  }
...
int N_puzzle<N>::lower_bound() const {
  ...
  return valid() ? manhattan_distance : mmul(N,mmul(N,N));
  ...
  for ( int i = 0; i < N; ++i ) {
    for ( int j = 0; j < N; ++j ) {
      if ( puzzle[i][j] != (static_cast<int8_t>(mdiv(count , mmul(N,N)).r) )) {
        ++result;
      }
