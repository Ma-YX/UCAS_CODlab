...
typedef struct Result {
  ...
  unsigned long branch_suc_count;
  unsigned long jump_count;
  unsigned long jump_suc_count;
} Result;
...
unsigned long _branch_cycle(){
  unsigned long * branch_count_addr;
  branch_count_addr = (unsigned long *)0x40021008;
  return *branch_count_addr;
}

unsigned long _branch_suc_cycle() {
  unsigned long * branch_suc_count_addr;
  branch_suc_count_addr = (unsigned long *)0x40022000;
  return *branch_suc_count_addr;
}

unsigned long _jump_cycle(){
  unsigned long * jump_count_addr;
  jump_count_addr = (unsigned long *)0x40022008;
  return *jump_count_addr;
}

unsigned long _jump_suc_cycle() {
  unsigned long * jump_suc_count_addr;
  jump_suc_count_addr = (unsigned long *)0x40023000;
  return *jump_suc_count_addr;
}
...
static void bench_prepare(Result *res) {
  ...
  res->branch_count = _branch_cycle();
  res->branch_suc_count = _branch_suc_cycle();
  res->jump_count = _jump_cycle();
  res->jump_suc_count = _jump_suc_cycle();
}
...
static void bench_done(Result *res) {
  ...
  res->branch_count = _branch_cycle() - res->branch_count;
  res->branch_suc_count = _branch_suc_cycle() - res->branch_suc_count;
  res->jump_count = _jump_cycle() - res->jump_count;
  res->jump_suc_count = _jump_suc_cycle() - res->jump_suc_count;
}
...
int main() {
  ...
  for (int i = 0; i < ARR_SIZE(benchmarks); i ++) {
    ...
    if (msg != NULL) {
      ...
      unsigned long branch_count = ULONG_MAX;
      unsigned long branch_suc_count = ULONG_MAX;
      unsigned long jump_count = ULONG_MAX;
      unsigned long jump_suc_count = ULONG_MAX;
      int succ = 1;
      for (int i = 0; i < REPEAT; i ++) {
        ...
        if (res.branch_count < branch_count) branch_count = res.branch_count;
        if (res.branch_suc_count < branch_suc_count) branch_suc_count = res.branch_suc_count;
        if (res.jump_count < jump_count) jump_count = res.jump_count;
        if (res.jump_suc_count < jump_suc_count) jump_suc_count = res.jump_suc_count;
      }
      ...
      printk("The number of branch operation counts: %u\n", branch_count);
      printk("The number of successful branch operation counts: %u\n", branch_suc_count);
      printk("The number of jump operation counts: %u\n", jump_count);
      printk("The number of successful jump counts: %u\n", jump_suc_count);
    }
  }
